...
- `datakonsistens-sjekk.ts` (daglig, i `link-check.yml`): sammenligner strukturerte felter med radens egen tekst. Fanger feilklassen der `age_group` sier «alle» mens beskrivelsen sier «aldersgrense 18 år» — usynlig for alle de andre lagene, fordi ingenting blir rødt. **Sperrende** sjekker skal alltid være null. **Målte** sjekker låser dagens nivå. Følger feilen katalogen, måles den som **andel**, ikke antall: `klokkeslett-spriker` sto på 291 stykker, og ble rød sju dager senere på 304 av 2 048 uten at andelen hadde endret seg. En grense som må skrus opp hver uke måler ingenting. Hever du en grense, skal det være en beslutning
