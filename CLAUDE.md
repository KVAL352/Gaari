...
- `/r/week/[startdate]/` — Weekly aggregate page (Mon-Sat) for batch posting. Three bulk download buttons: `reels.zip` (MP4s for Meta Business Suite), `stories.zip` (JPGs + handles.txt for manual IG posting), `carousels.zip` (1080×1080 + per-FB-group captions.txt). Two checklist sections (stories + carousel-per-group) with localStorage persistence so user can track what's been posted. Loads `week/{startdate}/manifest.json` from storage.
