...
- **Social post pipeline**: `scripts/social/` generates Instagram carousel images (Satori/Resvg, 1080x1080 PNG) + captions for scheduled collections. GHA cron at 07:00 UTC daily. Admin review at `/admin/social`. After generation, `post-to-bluesky.ts` auto-posts to `@gaari.no` via AT Protocol SDK (`@atproto/api`). Posts text + link card (OG image from gaari.no), natural conversational captions, no emojis. Phase 1 (building audience): only `denne-helgen` (Thu). Phase 2 (50+ followers): scale up to `i-kveld` (daily), `today-in-bergen` (daily), `this-weekend` (Thu), `gratis` (Mon), `konserter` (Mon). Bluesky URI stored in `social_posts.bluesky_uri`. Image compression via Resvg downscaling for Bluesky's 976KB blob limit. Bluesky also managed via MCP server + `/bluesky` skill in Claude Code.
...
- **Social Posts** (`social-posts.yml`): cron daily 07:00 UTC (08:00 CET / 09:00 CEST), 5min timeout. Runs `scripts/social/generate-posts.ts` (carousel images + captions for 8 collections), then `scripts/social/post-to-bluesky.ts` (auto-posts text + link card to @gaari.no via AT Protocol). Secrets: SUPABASE keys + BLUESKY_APP_PASSWORD.
