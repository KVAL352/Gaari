...
- `/[lang]/[collection]/` — 41 collection landing pages (14 evergreen + 13 seasonal + 14 festival). **Server-side loaded** with collection-specific filtering, ISR cached. Dynamic `[collection]` route — config in `$lib/collections.ts`, unknown slugs return 404. No EventDiscovery filter UI — clean hero + EventGrid + editorial copy + FAQ answer capsules (H2+p). JSON-LD `CollectionPage` + `ItemList` + `BreadcrumbList` + `FAQPage` schema, custom OG images. All 41 in sitemap (priority 0.8, daily). Seasonal/festival collections append current year to title/H1 for SEO. Festival collections filter by `source_url` domain (not date window) and support `maxPerVenue` override. Promoted placement logic runs after filtering — bubbles paying venue's events to the top and returns `promotedEventIds` to the page.
