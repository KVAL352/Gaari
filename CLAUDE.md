...
- **Social post pipeline**: `scripts/social/` generates Instagram carousel images (Satori/Resvg, 1080x1080 PNG) + captions for scheduled collections. GHA cron at 07:00 UTC daily. Admin review at `/admin/social`. Content generation only — no API posting yet. Bluesky account `@gaari.no` managed via MCP server + `/bluesky` skill in Claude Code.
