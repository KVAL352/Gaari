...
- **Link Check** (`link-check.yml`): daglig 08:00 UTC, to uavhengige jobber. `check-site` kjører i tillegg `dokumentstoerrelse-check.mjs` og `datakonsistens-sjekk.ts`, begge med `if: always()` så et rødt funn ikke skjuler det andre. `check-links` har egen 17-minutters frist, tre under jobbtaket, siden den ellers ble drept av GitHub før oppsummeringen rakk å kjøre.
