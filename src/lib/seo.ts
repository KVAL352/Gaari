...
export function generateOrganizationJsonLd(): string {
	const jsonLd = {
		...
		description: 'Gåri er en gratis arrangementskalender for Bergen, Norge. Vi samler arrangementer fra 54 kilder — konserter, utstillinger, teater, festival, mat og mer — på ett sted. Oppdatert to ganger daglig.',
		foundingDate: '2026',
		areaServed: {
			'@type': 'City',
			name: 'Bergen',
...
export function generateWebSiteJsonLd(lang: Lang): string {
	const jsonLd = {
		...
		url: BASE_URL,
		description: lang === 'no'
			? 'Gåri samler alle arrangementer i Bergen på ett sted — konserter, utstillinger, teater, mat og mer. Oppdatert to ganger daglig fra 54 kilder.'
			: 'Gåri aggregates all events in Bergen in one place — concerts, exhibitions, theatre, food and more. Updated twice daily from 54 sources.',
		inLanguage: ['nb', 'en'],
		about: {
			'@type': 'City',
			name: 'Bergen',
...
const FAQ_ITEMS: Record<Lang, Array<{ q: string; a: string }>> = {
	no: [
		{
			q: 'Hva er Gåri?',
			a: 'Gåri er en gratis arrangementskalender for Bergen som samler arrangementer fra 54 kilder — konsertsteder, teatre, museer, biblioteker, festivaler og billettsider — på ett sted. Oppdatert to ganger daglig.'
		},
		{
	...
	en: [
		{
			q: 'What is Gåri?',
			a: 'Gåri is a free event calendar for Bergen, Norway, collecting events from 54 sources — concert venues, theatres, museums, libraries, festivals and ticket platforms — in one place. Updated twice daily.'
		},
		{
