		...
		<div class="mb-8 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4" transition:slide={{ duration: 200 }}>
			...
			<form onsubmit={(e) => { e.preventDefault(); handleReminder(); }} class="flex gap-2">
				...
				<button
					...
					class="rounded-lg bg-[var(--color-accent)] px-4 py-2 text-sm font-semibold text-[var(--color-on-accent)] transition-colors hover:bg-[var(--color-accent-hover)] disabled:opacity-50"
				>
					{reminderSubmitting
