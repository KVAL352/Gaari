<script lang="ts">
	...
	let metaDesc = $derived($lang === 'no'
		? 'Hvordan Gåri samler inn eventdata fra 53 kilder i Bergen. Juridisk grunnlag, prinsipper og kontaktskjema for arrangører.'
		: 'How Gåri collects event data from 53 sources in Bergen. Legal basis, principles, and contact form for organizers.');
</script>
...
<div class="rounded-2xl bg-[var(--color-bg-surface)] px-6 py-10 shadow-[var(--shadow-sm)] sm:px-10">
	...
	<section class="mb-10">
		<p class="text-lg leading-relaxed">
			{#if $lang === 'no'}
				...
				informasjon om arrangementer i Bergen fra 53 kilder, slik at du kan finne alt som skjer på
				ett sted.
			{:else}
				We believe in transparency about how we collect event data. Gåri aggregates publicly
				available event information from 53 sources across Bergen, so you can find everything
	...
	<section class="mb-10 border-t border-[var(--color-border)] pt-8">
		<details class="rounded-lg border border-[var(--color-border)] p-5">
			<summary class="cursor-pointer text-xl font-semibold">
				{$lang === 'no' ? 'Datakilder (53 kilder)' : 'Data sources (53 sources)'}
			</summary>
