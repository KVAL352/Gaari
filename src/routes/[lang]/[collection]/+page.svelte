			...
			<li class="group list-none h-full">
				...
				>
					<div class="relative aspect-[16/9] overflow-hidden bg-[var(--color-surface)]">
						...
						<span class="absolute left-2 top-2 rounded-full bg-[var(--color-accent)] px-2.5 py-0.5 text-xs font-semibold text-[var(--color-on-accent)]">
	...
	{#if data.events.length === 0}
		<div class="py-10">
			<div class="mb-6 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-6 max-w-lg">
				...
				<a
					href="/{$lang}"
					class="inline-block rounded-xl bg-[var(--color-accent)] px-6 py-2.5 text-sm font-medium text-[var(--color-on-accent)] transition-colors hover:bg-[var(--color-accent-hover)]"
				>
					{$t('seeAllEvents')}
				</a>
			</div>
